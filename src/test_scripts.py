...
"""
Sanchari V4 - Pipeline Validation Tests (test_scripts.py)
...
import dataset
from predict import predict_sliding_window
...
class TestV4Pipeline(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_dir  = os.path.join(BASE_DIR, "test_env")
    ...
    def test_dataset_loading(self):
        ...
        ds = dataset.RoadSegmentationDatasetV4(
            image_dir=self.test_dir,
            mask_dir=self.test_dir,
            files=["test_img.jpg"],
            transform=dataset.get_transforms(train=False),
        )
